...
results_directory = 
  "../data/Meta_Analysis_Results/Results_2025-02-15/"

results_by_type_directory = 
"../data/Meta_Analysis_Results/Results_BY_TYPE_2025-02-15/"


results_by_age_directory = 
  "../data/Meta_Analysis_Results/Results_2025-02-17_age_adjust/"

results_by_age_and_type_directory = 
"../data/Meta_Analysis_Results/Results_BY_TYPE_2025-02-17_age_adjust/"
```
...
\markup{1. $E_S$: The percent reduction in the rate of newly acquired SARS-CoV-2 infections among \markup{individuals 18-64} with vaccine-induced and/or infection-induced immunity relative to those with no pre-existing immunity (the naive population).
2. $E_D$: The percent reduction in the rate of symptomatic COVID-19 disease among \markup{individuals 18-64} with vaccine-induced and/or infection-induced immunity relative to the naive population. 
3. $E_M$: The percent reduction in the rate of severe COVID-19 disease among \markup{individuals 18-64} with vaccine-induced and/or infection-induced relative to the naive population. \markup{Severe in this context means an infection leading to hospitalization and/or death.}}
...
\markup{As these model are intended to be descriptive rather than project future effectiveness, we restrict our analyses to the time frame with the gratest amount of data. For models 1-4, our initial and waned immunity refers to protection at two and 26 weeks post ICE, respectively. For model 5, waned immunity refers to protection at 16 weeks post ICE, owing to a relative lack of data outside of that time frame. To test for the robustness of our results to model assumptions we also considered a log-transformed version of our model replacing $E$ with $\log(1 - E)$. These results are provided in the supplementary materials.}
```{r Model-Table}
...
```{r Strain-Figure, fig.width = 14, fig.height = 9, fig.cap = "Protection against infection, symptomatic disease, and severe disease by infecting strain and time since most recent vaccination relative to an immune-naive population. Size of each dot is proportional to weighting in the analysis.", dev = "tiff", dpi = 500, out.width = "5.83in", out.height = "3.75in"}
#library(viridis)
...
fig_one_function = function(x, type = "All"){
  ...
  geom_line(aes(y = pred), lwd = 1, data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)]) +
  geom_line(aes(y = ci.lb), lwd = 0.5, data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)], linetype = "dashed") +
  geom_line(aes(y = ci.ub), lwd = 0.5, data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)], linetype = "dashed") +
    geom_ribbon(aes(ymin = ci.lb, ymax = ci.ub, x = MIN_TIME), 
                data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)], color = NA, alpha = 0.3, size = 1) +
  guides(size = "none")+
...
```{r Booster-Figure, echo=FALSE, fig.width = 12, fig.height = 7,fig.cap = "Protection against infection, symptomatic disease, and severe disease by booster status and time since most recent vaccination relative to an immune-naive population. Top and bottom rows show protection against pre-Omicron and Omicron variants, respectively. Size of each dot is proportional to weighting in the analysis.", dev = "tiff", dpi = 500, out.width = "5.83in", out.height = "3.4in"}
...
#   geom_line(aes(y = -pred), data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)], lwd = 1) +
#   geom_line(aes(y = -ci.lb), data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)], lwd = 0.5, linetype = "dashed") +
#   geom_line(aes(y = -ci.ub_truncated), data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)], 
#             lwd = 0.5, linetype = "dashed") +
#   geom_ribbon(aes(ymin = -ci.lb, ymax = -ci.ub_truncated, x = MIN_TIME), 
#               data.temp[AGE_GROUP == "Adult" | is.na(AGE_ADJUST)], color = NA, alpha = 0.3, size = 1) +
#   guides(size = "none")+
...
ggplot(data.temp,
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        legend.position = "bottom") +
  theme(text = element_text(size = 20)) +
  geom_line(aes(y = pred), data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], lwd = 1) +
  geom_line(aes(y = ci.lb), data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], lwd = 0.5, linetype = "dashed") +
  geom_line(aes(y = ci.ub_truncated), data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], lwd = 0.5, linetype = "dashed") +
  geom_ribbon(aes(ymin = ci.lb, ymax = ci.ub_truncated, x = MIN_TIME), data = data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], color = NA, alpha = 0.3, size = 1) +
  guides(size = "none")+
  labs(x = "Minimum number of weeks since vaccination", y="Efficacy", col = "Dose", fill = "Dose") +
  facet_grid(STRAIN~TYPE2,
             labeller = labeller(
...
against pre-Omicron variants was similar following natural infection and vaccination whether the outcome was reinfection ($E_S$, Figure \@ref(fig:Infection-Figure)A), `r rtable_infection_bestfit_only[TYPE == "S" & STRAIN == "Pre-Omicron" & IMM == "I"]$print.value.2` vs  `r rtable_infection_bestfit_only[TYPE == "S" & STRAIN == "Pre-Omicron" & IMM == "V"]$print.value.2`, or
...
ggplot(data.temp,
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        legend.position = "bottom") +
  theme(text = element_text(size = 20)) +
  geom_line(aes(y = pred), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], lwd = 1) +
  geom_line(aes(y = ci.lb), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], lwd = 0.5, linetype = "dashed") +
  geom_line(aes(y = ci.ub_truncated), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], lwd = 0.5, linetype = "dashed") +
  geom_ribbon(aes(ymin = ci.lb, ymax = ci.ub_truncated, x = MIN_TIME), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], color = NA, alpha = 0.3, size = 1) +
  guides(size = "none")+
...
RFIG_INF_DATA_pstrain[
  BEST_FIT_INF_pstrain, 
  on = .(TYPE, STRAIN, method, version, STUDY_TYPE_SIMPLE, AGE_ADJUST)
  ][OUTPUT == "Predicted" & !is.na(PSTRAIN) & version == "linear" & STUDY_TYPE_SIMPLE == "All"
    ]->data.temp
...
ggplot(data.temp,
  ...
  theme(text = element_text(size = 20)) +
  geom_line(aes(y = pred), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], lwd = 1) +
  geom_line(aes(y = ci.lb), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], 
            lwd = 0.5, linetype = "dashed") +
  geom_line(aes(y = ci.ub), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], 
            lwd = 0.5, linetype = "dashed") +
  geom_ribbon(aes(ymin = ci.lb, ymax = ci.ub, x = MIN_TIME), 
              data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], color = NA, 
              alpha = 0.3, size = 1) +
  guides(size = "none")+
  labs(x = "Minimum number of weeks since immune conferring event", y="Efficacy") +
...
and without prior infection (Figure \@ref(fig:Hybrid-Boosted-Figure) and Supplementary Table S25). At two weeks following a booster dose, individuals with a prior infection had significantly greater $E_S$, `r rtable_infection_hboosted_bestfit[TYPE == "S" & Infected == "TRUE"]$print.value.2` vs `r rtable_infection_hboosted_bestfit[TYPE == "S" & Infected == "FALSE"]$print.value.2` and $E_D$, `r rtable_infection_hboosted_bestfit[TYPE == "D" & Infected == "TRUE"]$print.value.2` vs `r rtable_infection_hboosted_bestfit[TYPE == "D" & Infected == "FALSE"]$print.value.2` than those with no prior infection. Differences at 30 weeks were not significant.
...
ggplot(data.temp,
  ...
  geom_line(aes(y = ci.ub), data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], 
            lwd = 0.5, linetype = "dashed") +
  geom_ribbon(aes(ymin = ci.lb, ymax = ci.ub, x = MIN_TIME), 
              data.temp[AGE_GROUP == "Adult" | is.na(AGE_GROUP)], 
